        ...
        <div className="max-w-4xl mx-auto relative min-h-[400px]">
          ...
          {t.experience.map((item, index) => {
            ...
            return (
              <ScrollReveal3D
                key={item.id}
                variant={isRight ? "slideLeft" : "slideRight"}
                delay={index * 0.1}
                className={`relative z-10 mb-16 pl-8 md:pl-0 ${isRight ? 'md:ml-auto md:pl-16' : 'md:mr-auto md:pr-16 md:text-right'} md:w-1/2 group`}
              >
                {/* Timeline Dot */}
